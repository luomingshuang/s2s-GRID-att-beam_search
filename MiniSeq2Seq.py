...
class Seq2Seq(nn.Module):
    ...
    def beamsearchdecoder(self, beam_size, output, context, hidden, encoder_output, attn_weights):
        topk = output.data.topk(beam_size)
        samples = [[] for i in range(beam_size)]
        #output = self.decoder(output, )
        for index in range(beam_size):
            topk_prob = topk[0][0][index]
            topk_index = topk[1][0][index]
            samples[index] = [[topk_index], topk_prob, 0, 0, context, hidden, attn_weights, encoder_output]
